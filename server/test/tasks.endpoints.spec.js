...
describe("Task Endpoints", function() {
    ...
    describe("DELETE /api/tasks/:task_id", () => {
        ...
        it("Deletes a Task when given an ID", () => {
            let testTask = testTasks[0];
        ...
        it("returns 404 if the ID does not exist", () => {
            ...
            return supertest(app)
                ...
                .then((res) => {
                    ...
                    expect(res.error.text).to.equal(`Please select a valid Task.`);
                })
        });
    });
    ...
    describe("PATCH /api/tasks/:task_id", () => {
        ...
        it("updates Task to Completed", () => {
            ...
            return supertest(app)
                .patch(`/api/tasks/${testTask.task_id}`)
                .set("Authorization", helpers.makeAuthHeader(testUser))
                .expect(200)
                .expect(res => {
                    db
                    ...
                    .first()
                    .then(row => {
                        expect(row.completed).to.eql("Y")
                    })
                })
        });
        
        it("returns 404 if the Task ID does not exist", () => {
            let fakeId = "12345";
            let testUser = testUsers[0];
            ...
            return supertest(app)
                ...
                .then((res) => {
                    expect(res.status).to.equal(404);
                    expect(res.error.text).to.equal(`"That Task does not exist."`);
                })
        });
    });
})